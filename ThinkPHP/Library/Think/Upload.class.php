 ...
 * @property mixed $saveName
 * @property string $savePath
 * @property bool $autoSub
 * @property mixed $subName
 * @property string $removeTrash
...
class Upload
{
    ...
    public function uploadToStorage($config)
    {
        if (false === $ret = $this->uploader->upload($config)) {
    ...
    public function upload($files = '')
    {
        ...
        foreach ($files as $key => $file) {
            ...
            //保存文件 并记录保存成功的文件 save方法上传成功后 $file['url'] 必须赋值上传成功的url
            //modified by tiancheng
            if ($ret = $this->uploader->save($file, $this->replace)) {
