...
class Mongo extends Db{
     ...
     * @access public
     */
    public function connectd($config='',$linkNum=0) {
    ...
    public function connect() {
         ...
         if ( !isset($this->linkID[$linkNum]) ) {
            ...
            try{
                ...
                // var_dump($this->linkID[$linkNum]);
                // print_r($this->linkID[0]->getConnections());
                //设置主从读取模式，大多数情况下，从从节点读取，在只有主节点的情况下，读取主节点，RP_SECONDARY,只读取从库，没有丛库的情况下，会报错
                if(C('DB_RW_SEPARATE')) $this->linkID[$linkNum]->setReadPreference(\MongoClient::RP_SECONDARY_PREFERRED);
            }catch (\MongoConnectionException $e){
                E($e->getmessage());
            }
            // 标记连接成功
