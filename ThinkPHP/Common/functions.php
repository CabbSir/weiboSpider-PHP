...
//    }
    ...
    if (isset($options['counter'])) {
        if ('+' == $value) {
            if (is_int($options['step'])) {
                return $cache->incre($name, $options['step']);
            } else {
                return $cache->incre($name);
            }
        } else {
            if (is_int($options['step'])) {
                return $cache->decre($name, $options['step']);
            } else {
                return $cache->decre($name);
            }
        }
    }

    if (isset($options['get_keys'])) {
        return $cache->keys($name);
    }
 ...
 * 参数验证
 */
function V($rules = null, $messages = null) {
    if(!\Common\Util\Validator::validate($rules, $messages)){
        E(\Common\Util\Validator::getError(),C('PARAM_ERROR'));
    }
}