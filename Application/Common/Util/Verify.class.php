...
class Verify {
     ...
     */
    public function check($code, $id = '', $session = null) {
    ...
    public function entry($id = '') {
        ...
        // 图片高(px)
        $this->imageH || $this->imageH = $this->fontSize * 2.4;
        ...
        for ($i = 0; $i<$this->length; $i++) {
            if($this->useZh) {
                ...
                $codeNX += mt_rand($this->fontSize*1.0, $this->fontSize*1.4);
                // 写一个验证码字符
                $this->useZh || imagettftext($this->_image, $this->fontSize, mt_rand(-20, 20), $codeNX, $this->fontSize*1.6, $this->_color, $this->fontttf, $code[$i]);
            }
        }
        ...
        // 保存验证码
        $key = $this->authcode($this->seKey);
        \Think\Log::record(strtoupper(implode('', $code)), 'DEBUG');
...
//        imagedestroy($this->_image);
    ...
    private function _writeNoise() {
        ...
        /*for($i = 0; $i < 10; $i++){
            ...
            for($j = 0; $j < 5; $j++) {
                // 绘杂点
                imagestring($this->_image, 3, mt_rand(-10, $this->imageW),  mt_rand(-10, $this->imageH), $this->_codeNoise[mt_rand(0, 9)], $noiseColor);
            }
        }*/
    }
